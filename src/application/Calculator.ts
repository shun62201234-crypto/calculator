...
/*memo
---
private left: number | null = null;のnull = null意味は、初期状態では空（null）とすること。
訳すと「left は数値かもしれないし空かもしれない。ただし初期状態では空（null）として始める」
なぜこのように記載するのかというと、「未入力状態」と「0」は別物として扱うため、
初期状態：まだ左辺が決まってない → null
ユーザーが「3」と入力 → 3
計算後リセット → null or 次の計算で再利用
「型定義」と「初期値設定」がセットになっている。
private operator: Operation | null = null;も同様に訳すと、
「今の計算に使う演算子を保持する変数で、まだ決まっていない状態は null（null）で表す」
---
*/
/*memo
---
以下のコードはなぜreturnだけで成立するのかについては、
以降の処理が“全部その入力を前提にしている”から、ここで止めれば後ろが安全になるから。
「条件に合わないなら、後続ロジックを“考える必要すらない状態”にする」思想のガード節となる。
if (NG条件) return;「この関数の“正常ルート”に入れる条件だけ残す」理由を一言でいうと
「後ろのコードが“正常入力専用”として設計されているから」
---
public handleInput(token: KeyToken): void {
    if (!this.canAcceptInput(token)) {
        return;
